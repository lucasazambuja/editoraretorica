// Template Fastcommerce [06/2018][2]
...
var sF$=(function(){
  ...
  function fnShowProd(Price,OriginalPrice,Cod,iMaxParcels,ProductID){
    //fun��o que mostra o pre�o do produto, parcelamento, desconto � vista e badge (home e lista)
    iPL++;
    var idPrice=fnGetID("idProdPrice"+ProductID);
    var sPrice="";
    if(Price==0 && OriginalPrice==0){
      if(idPrice)idPrice.innerHTML="<div class=\"prices\"><br><div class=\"price font-bold\"><div class=currency><a href='/faleconosco.asp?idloja="+FC$.IDLoja+"&assunto=Consulta%20sobre%20produto%20(C�digo%20"+Cod+")' target='_top' >Consulte-nos</a></div></div></div>";
      return void(0);
    }
    ...
    if(Price>0 && iDescontoAvista>0){
      ...
      if(oProdDesc)oProdDesc.innerHTML="� vista <b>"+FormatPrice(Price*((100-iDescontoAvista)/100),FC$.Currency)+"</b>";
    }
    //badge
  ...
  function fnShowPageHistory(oHistoryPages){
    ...
    if(oPageHistory){
      ...
      if(sBar!=""){sPageHistory+="<div id='idDivPageHistory'><div id='idPageHistoryFC'><div id='idTitPageHistory'><h3>Meu hist�rico de navega��o:</h3></div><ul id='idListPageHistoryFC'>";}  
  ...
  function fnLoginUserName(NameUser,PicUser){
    ...
    if(NameUser==""){
      document.getElementById('fc-loginInfo').innerHTML = "<a href='/cadastro.asp?idloja="+FC$.IDLoja+"&pp=3&passo=1&sit=1'><img src="+FC$.PathImg+"iconuser.svg width='30' height='30'></a>";
  ...
  // Fixed Grid Product Page
  function fnFixedGridProductPage(){
    if(FC$.Page=="Products"){
      (function(jQuery) {
        function mediaSize() { 
          if (window.matchMedia('(min-width:1025px)').matches) {
            if(jQuery('.det-product-img-name-container-two').offset().top + jQuery('.det-product-img-name-container-two').height() >= jQuery('.det-product-fixed-remove').offset().top - 10)
              jQuery('.det-product-img-name-container-two').css('position', 'absolute');
            if(jQuery(document).scrollTop() + window.innerHeight < jQuery('.det-product-fixed-remove').offset().top)
              jQuery('.det-product-img-name-container-two').css('position', 'fixed');
        
          } else {
            if(jQuery('.det-product-img-name-container-two').offset().top + jQuery('.det-product-img-name-container-two').height() >= jQuery('.det-product-fixed-remove').offset().top - 10)
              jQuery('.det-product-img-name-container-two').css('position', 'static');
            if(jQuery(document).scrollTop() + window.innerHeight < jQuery('.FooterMainBarFC').offset().top)
              jQuery('.det-product-img-name-container-two').css('position', 'static');
          }
        };
...
function FuncButtonAddToWL(idp,bAdd){
  ...
  if(bProdDet){
    if(bAdd==true)sCont="<a title='Produto adicionado a lista de desejos. Clique aqui para ir para a lista de desejos.' href=\"/account.asp?idloja="+ FC$.IDLoja +"&wishlist=1#Wishlist\"><span class=\"icon-share-wishlist-on-det-product\"></span></a>";
    else sCont="<a title='Clique aqui para adicionar o produto a lista de desejos' onclick=\"WL$.fnAddToWishlist("+idp+")\" rel=\"nofollow\"><span class=\"icon-share-wishlist-off-det-product\"></span></a>";
    return sCont;
  }else{
    if(bAdd==true)sCont="<a title='Produto adicionado a lista de desejos. Clique aqui para ir para a lista de desejos.' href=\"/account.asp?idloja="+ FC$.IDLoja +"&wishlist=1#Wishlist\"><span class=\"icon-share-wishlist-on\"></span></a>";
    else sCont="<a title='Clique aqui para adicionar o produto a lista de desejos' onclick=\"WL$.fnAddToWishlist("+idp+")\" rel=\"nofollow\"><span class=\"icon-share-wishlist-off\"></span></a>";
    return sCont; 
  }
}
...
function ShowCartOnPage(IDLoja,iErr,sMsg,sCartText,sCheckoutText,este){
  if(!IsFramePage){
    ...
    if(iErr==0){
      sHTML+="<tr height=45>";
      sHTML+="<td valign=top align=center style=cursor:pointer onclick=top.location.href='/addproduto.asp?idloja="+ IDLoja +"'><a href='/addproduto.asp?idloja="+ IDLoja +"'><span class='fc-cart-onpage-cart-txt'>Ir para o carrinho</span></a></td>";
...
function fnShowCEP(IDProd){
  if(FC$.TypeFrt==3 || FC$.TypeFrt==4){
    ...
    sCEP+="  <div id='idDivTitCEP'><span class='font-bold'>Veja o valor do frete</span></div>";
...
var Cart$=(function(){
  ...
  function fnGetShippingValueCart(iValorCesta){
    ...
    FCLib$.SetCookie("CEP"+FC$.IDLoja,sCEP);
    if(sCEP==""){document.getElementById("idShippingValue").innerHTML="<span style=color:#990000;>CEP inv�lido</span>";}
    else{
  ...
  function fnProcessXMLCEPCart(obj,iValorCesta){
    ...
    var OptValue=ReadXMLNode(obj,"Opt1Value");
    var OptObs=ReadXMLNode(obj,"Opt1Obs");
    //oShippingObs.innerHTML="<b>"+ OptName +"</b><br><span class=ObsFreightCalc>"+ OptObs +"</span>"; //Insere observa��es
